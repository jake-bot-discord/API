...
export const success = async (app: FastifyInstance, req: any, rep: FastifyReply) => {
    try {
        ...
        // if (!checkoutSessionId || !checkoutSession || !stripeCustomer || !userId) return rep.status(500).send("internal error")
        ...
        return rep.status(302).redirect(`${process.env.APP_URL}/checkout/complete`).send("ok")
    } catch (err) {
        console.log(err)
